...
/**
 * Batch fetch for the 30-candidate universe with real Twelve Data market data.
 * If a ticker's fetch fails or has insufficient history, it is marked ineligible
 * with the error message shown in the universe table, and execution continues
 * with the remaining tickers. Never substitutes synthetic prices.
 * Live mode requires a Twelve Data API key.
 */
export async function fetchUniversePrices(
  ...
  if (!apiKey || apiKey.trim() === '') {
    throw new Error('Twelve Data API key is required to run in Live Screener Mode. Please enter your Twelve Data API key.');
  }
      ...
      chunk.map(async (ticker) => {
        ...
        try {
          ...
          if (bars && bars.length >= 210) {
            ...
            errors[ticker] = `Insufficient price history (${bars ? bars.length : 0} < 210 trading days)`;
          }
        } catch (err: any) {
          errors[ticker] = err.message || `Fetch failed for ${ticker}`;
        } finally {
          completed++;
          if (onProgress) {
            onProgress({
